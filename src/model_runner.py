...
class ModelRunner:
    ...
    def __init__(
        ...
        method: str,
        model_path: str,
        load_model: Callable[[str, torch.device], nn.Module], # a function that can load a model and takes the path to the model
        forward_model: Callable[[nn.Module, torch.Tensor], list[Any]] | None = None, # (optional) function to forward a model
        ...
        *,
        ...
        with torch.no_grad():
            if self.forward_model is None:
                ...
                y_pad = self.forward_model(self.model, x_pad)
        torch.cuda.synchronize() if self.device.type == "cuda" else None
        t1 = time.time()
