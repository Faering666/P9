...
class Net(torch.nn.Module):
    ...
    def forward_with_f0(self, f0, input_mask=None):
        ...
        f = self.initial(torch.cat([f0, Phi], dim=1)) if Phi is not None else self.initial(f0)
        ...
        for i in range(self.stage):
            ...
            f = self.net_stage[2 * i](z, Phi if Phi is not None else torch.ones_like(f0))
