...
class ModelRunner:
    ...
    def __init__(
        ...
        # ---- build/load model ----
        self._log(
            f"Building model '{self.method}'", 
            True
        )
        self.model = self._load_model(self.method, self.model_path)
    ...
    # ---------- Model loader --------
    def _load_model(self, method: str, model_path: str):
        """
        Decide how to construct/restore the model based on the file extension.
        - For .pth / other: delegate to model_generator (unchanged behavior).
        - For .pkl: load the pickled model or state dict.
        """
        _, ext = os.path.splitext(model_path.lower())

        if ext == ".pth":
            return model_generator(method, model_path).to(self.device).eval()

        elif ext == ".pkl":
            self._log("Detected .pkl model file. Loading via pickle...", True)

            obj = torch.load(
                f="./lib/model_runner/model_zoo/model_S.pkl",
                map_location="cpu",
                weights_only=False
            )
            ...
            if method_size == 's':
                ...
                raise ValueError("Method tail must be 's', 'm' or 'l' to specify which model size to use.")
        ...
        raise TypeError(
            "File passed is not of type '.pkl' or '.pth'."
        )
